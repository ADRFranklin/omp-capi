...
public:
    ...
    /// Ensure capacity for a new total bit length without crossing allocator
    /// boundaries.  Intended for synchronous network bridge adapters.
    inline void EnsureBitCapacity(int bitLength)
    {
        if (bitLength > numberOfBitsUsed)
            AddBitsAndReallocate(bitLength - numberOfBitsUsed);
    }

    inline int GetNumberOfBitsAllocated(void) const { return numberOfBitsAllocated; }
